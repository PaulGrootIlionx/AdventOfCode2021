...
begin
	set @fuelconsumption = @fuelconsumption + @step;
	set @step = @step + 1;
	insert into #fuelrequirements values (@minposition,@fuelconsumption);
...
begin
	...
	select
		@fuelconsumption = sum(req.FuelRequirement)
	from
		(select
			case
				when Position < @minposition then @minposition - Position
				else Position - @minposition
			end as Moves
		from #input) src
	join #fuelrequirements req
		on src.Moves = req.Moves;

	insert into #output values (@minposition,@fuelconsumption);
