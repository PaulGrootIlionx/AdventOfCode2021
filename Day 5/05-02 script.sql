...
select
	AutoID
	,cast(trim(left([StartPoint],charindex(',',[StartPoint])-1)) as int) as [StartPointY]
	,cast(trim(substring([StartPoint],charindex(',',[StartPoint])+1,99)) as int) as [StartPointX]
	,cast(trim(left([EndPoint],charindex(',',[EndPoint])-1)) as int) as [EndPointY]
	,cast(trim(substring([EndPoint],charindex(',',[EndPoint])+1,99)) as int) as [EndPointX]
into #data
from
	(select
		*
...
--add vertical lines to worklist
insert into #worklist
select
	*
	,0 as XIncrement
	,case
		when StartPointY > EndPointY then -1
		else 1
	end as YIncrement
	,case
		when StartPointY > EndPointY then cast(StartPointY as int) - cast(EndPointY as int)
		else cast(EndPointY as int) - cast(StartPointY as int)
	end + 1 as diff
from #data
where
	StartPointX = EndPointX;

--add horizontal lines to worklist
insert into #worklist
select
	*
	,case
		when StartPointX > EndPointX then -1
		else 1
	end as XIncrement
	,0 as YIncrement
	,case
		when StartPointX > EndPointX then cast(StartPointX as int) - cast(EndPointX as int)
		else cast(EndPointX as int) - cast(StartPointX as int)
	end + 1 as diff
from #data
where
	StartPointY = EndPointY;

--add diagonal lines to worklist
insert into #worklist
select
	*
	,case
...
where
	StartPointY <> EndPointY
	and StartPointX <> EndPointX;

while (select count(*) from #worklist where diff > 0) > 0
begin

	update o
		set Counts = o.Counts + u.Counts
	from #output o
	join
		(select
			StartPointX, StartPointY, count(*) Counts
		from #worklist
		where
			diff > 0
		group by
			StartPointX, StartPointY) u
		on o.X = u.StartPointX
		and o.Y = u.StartPointY;

	update #worklist
		set StartPointX = StartPointX + XIncrement
		   ,StartPointY = StartPointY + YIncrement
		   ,diff = diff - 1
	where
			diff > 0;
...
declare @X int;
declare @row nvarchar(1000);

open print_cur;

fetch next from print_cur into @X;

while @@FETCH_STATUS = 0
begin
	select
		@row = concat(@X,' ', (select
			concat('',case Counts when 0 then '.' else convert(nvarchar,Counts) end,'')
		from #output
		where
			X = @X
		order by
